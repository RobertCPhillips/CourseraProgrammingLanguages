...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1,x2)
      aXstart = (y1 < seg.y1) ? x1 : seg.x1
      aYstart = (y1 < seg.y1) ? y1 : seg.y1
      aXend = (y1 < seg.y1) ? x2 : seg.x2
      aYend = (y1 < seg.y1) ? y2 : seg.y2
      bXstart = (y1 < seg.y1) ? seg.x1 : x1
      bYstart = (y1 < seg.y1) ? seg.y1 : y1
      bXend = (y1 < seg.y1) ? seg.x2 : x2
      bYend = (y1 < seg.y1) ? seg.y2 : y2
      
      if real_close(aYend,bYstart)
        Point.new(aXend,aYend)
      elseif aYend < bYstart
        NoPoints.new
      elseif aYend > bYend
      ...
      aXstart = (x1 < seg.x1) ? x1 : seg.x1
      aYstart = (x1 < seg.x1) ? y1 : seg.y1
      aXend = (x1 < seg.x1) ? x2 : seg.x2
      aYend = (x1 < seg.x1) ? y2 : seg.y2
      bXstart = (x1 < seg.x1) ? seg.x1 : x1
      bYstart = (x1 < seg.x1) ? seg.y1 : y1
      bXend = (x1 < seg.x1) ? seg.x2 : x2
      bYend = (x1 < seg.x1) ? seg.y2 : y2
       
      if real_close(aXend,bXstart)
        Point.new(aXend,aYend)
      elseif aXend < bXstart
        NoPoints.new 
      elsef aXend > bXend
        LineSegment.new(bXstart,bYstart,bXend,bYend) 
      else 
